...
def hutao_docs_parser(url: str):
    uri_path = urlparse(url).path
    if "/zh/" in uri_path:
        lang = "zh"
    else:
        lang = "en"
    hashed_uri = hashlib.md5(uri_path.encode()).hexdigest()
    r = requests.get(url)
    ...
    if this_title and this_description:
        if ENABLE_DESCRIPTION:
            hashed_uri = hashed_uri + "1"
            make_open_graph_image_with_description(this_title, this_description, hashed_uri, lang)
        else:
            hashed_uri = hashed_uri + "0"
            make_open_graph_image_with_no_description(this_title, hashed_uri, lang)
    elif this_title and not this_description:
        hashed_uri = hashed_uri + "0"
        make_open_graph_image_with_no_description(this_title, hashed_uri, lang)
    else:
...
if __name__ == "__main__":
    ...
    hutao_docs_parser("https://deploy-preview-117--snap-hutao-docs.netlify.app/zh/features/game-launcher.html")
